...
client.on("inviteCreate", async invite => {
    ...
    await channel.send(Translator.get(invite).translate("embeds.invitetracker.invite_created", invite.code, invite.inviter.tag))
...
client.on("inviteDelete", async invite => {
    ...
    await channel.send(Translator.get(invite).translate("embeds.invitetracker.invite_deleted", invite.code))
        .catch(() => cleanTrackedGuild(invite.guild.id));
});
...
client.on("guildMemberAdd", async member => {
    ...
    let translator = Translator.get(member);
    try {
